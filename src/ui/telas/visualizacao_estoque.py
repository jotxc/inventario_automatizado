...
from ui.tema import (
    ...
    COR_FUNDO, COR_CARD, COR_BORDA, COR_AVISO, COR_ERRO, COR_ERRO_HOVER
)
...
class TelaVisualizacaoEstoque(ctk.CTkFrame):
    ...
    def __init__(self, master, controller, ao_exibir_resultado=None, ao_exibir_visao_geral=None):
        ...
        self.after(0, self._iniciar_carregamento)
    ...
    def _animar_carregamento(self):
        valor = self.loading_progress.get()
        if valor < 0.95:
            self.loading_progress.set(valor + 0.02)
        else:
            self.loading_progress.set(0)
        self.update_idletasks()
        self._animacao_id = self.after(30, self._animar_carregamento)
    ...
    def _criar_filtros(self):
        ...
        self.entry_limite.grid(row=0, column=col, padx=(0, 20), pady=pad_y, sticky="w")
        col += 1
        ...
        self.switch_baixas.grid(row=0, column=col, padx=(0, 20), pady=pad_y, sticky="w")
        col += 1
    ...
    def _abrir_exclusao_descricao(self):
        ...
        if popup.resultado is not None:
            ...
            if self.ultima_consulta_sem_exclusao is not None:
                dados = self.controller.aplicar_exclusao(self.ultima_consulta_sem_exclusao)
                saved_sort = self.tabela.coluna_ordenada
                if dados.empty:
                    self.tabela.mostrar_mensagem(
                        "Todas as descri\u00e7\u00f5es foram exclu\u00eddas. Use \"\u2716 Limpar Exclus\u00e3o\" para redefinir."
                    )
                else:
                    self.tabela.carregar(dados)
                    if saved_sort:
                        self.tabela._ordenar_por(saved_sort)
    ...
    def _limpar_exclusao_descricao(self):
        ...
        if self.ultima_consulta_sem_exclusao is not None:
            saved_sort = self.tabela.coluna_ordenada
            self.tabela.carregar(self.ultima_consulta_sem_exclusao)
            if saved_sort:
                self.tabela._ordenar_por(saved_sort)
            self._atualizar_totais(self.ultima_consulta_sem_exclusao)
    ...
    def _criar_barra_acoes(self):
        ...
        self.label_totais.grid(row=0, column=0, padx=(15, 10), pady=15, sticky="w")
        ...
        self.botao_importar.grid(row=0, column=1, padx=(0, 8), pady=13, sticky="e")
        ...
        self.botao_gerar.grid(row=0, column=4, padx=(0, 15), pady=13, sticky="e")
    ...
    def _executar_consulta(self, criterio, criterio_sec, tipo, limite, modo_sem_maquina):
        try:
            ...
            erro = str(e)
            self.after(0, lambda: self._erro_consulta(erro))
    ...
    def _pos_consulta(self, resultado, modo_sem_maquina=False):
        ...
        self._atualizar_totais(resultado)
        self.botao_aplicar.configure(state="normal", text="\U0001F50D  APLICAR")

    def _atualizar_totais(self, dados):
        total_lotes = len(dados)
        total_posicoes = int(dados["quantidade_posicoes"].sum()) if not dados.empty else 0
        total_valor = dados["valor_lote"].sum() if not dados.empty else 0
